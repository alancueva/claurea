...
export default function SectionProductos() {
    const productos = [
        ...
        },
        {
          ...
          link: "/pages/products/serums",
          beneficios: ["Nutrición profunda", "Crecimiento y volumen", "Reparación de daños"]
        }
      ];
      ...
      const handleProductClick = (link: string) => {
        // En una aplicación real de Next.js usarías: router.push(link)
        router.push(link);
      };
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto">
            {productos.map((producto) => (
              <div
                key={producto.id}
                className={`group bg-white rounded-2xl shadow-lg overflow-hidden cursor-pointer transition-all duration-300 transform ${
                  hoveredProduct === producto.id ? 'scale-105 shadow-2xl' : 'hover:shadow-xl'
                }`}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center text-blue-600 group-hover:text-blue-700 font-semibold transition-colors">
                      ...
                      <ArrowRight className={`w-5 h-5 transition-transform duration-300 ${
                        hoveredProduct === producto.id ? 'translate-x-2' : ''
                      }`} />
                    </div>
                  </div>
                  </div>
